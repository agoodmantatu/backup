...
function Toggle({ checked, onChange, id }) {
  return (
    <button
      ...
      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${checked ? 'bg-[#D4AF37]' : 'bg-gray-200'}`}
    >
    ...
    <AppLayout title="Accessibility">
      ...
      <div className="max-w-xl mx-auto space-y-6 p-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <p className="font-bold text-[#1E3A5F] mb-1">🔤 Text Size</p>
          ...
          <div className="flex gap-2">
            {TEXT_SIZES.map(size => (
              <button
                ...
                className={`flex-1 py-2 rounded-xl text-sm font-semibold border-2 transition ${textSize === size ? 'border-[#D4AF37] bg-[#FDF6E3] text-[#D4AF37]' : 'border-gray-200 text-gray-500 hover:border-gray-300'}`}
              >
                {size}
              </button>
            ))}
        ...
        {settings.map(s => (
          <div key={s.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-start gap-4">
            ...
            <div className="flex-1">
              <label htmlFor={s.id} className="font-bold text-[#1E3A5F] cursor-pointer">{s.title}</label>
        ...
        <p className="text-xs text-center text-gray-400 pb-4">
          Need additional support? Email <span className="text-[#D4AF37]">accessibility@tryiteducations.net</span>
        </p>
      </div>
    </AppLayout>
  )
