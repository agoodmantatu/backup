...
export default function AdminUserManager() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#F8FAFC' }}>
      <div style={{ background:'linear-gradient(135deg,#1E3A5F,#0F2140)', padding:'20px 32px' }}>
        ...
        <p style={{ fontFamily:'Poppins,sans-serif', fontWeight:900, color:'#D4AF37', fontSize:22, margin:0 }}>👥 User Manager</p>
      </div>
      ...
      <div style={{ padding:'24px 32px' }}>
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign:'center', padding:60, color:'#94A3B8', background:'#fff', borderRadius:18, border:'1.5px solid #E2E8F0' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {filtered.map(u=>(
              <div key={u.id} style={{ background:'#fff', borderRadius:14, border:'1.5px solid #E2E8F0', padding:'12px 16px', display:'flex', alignItems:'center', gap:12, flexWrap:'wrap' }}>
                ...
                <div style={{ flex:1, minWidth:120 }}>
                  <p style={{ fontWeight:700, color:'#1E3A5F', fontSize:13, margin:'0 0 2px' }}>{u.name}</p>
                  <p style={{ color:'#94A3B8', fontSize:11, margin:0 }}>{u.email} · {u.role} · joined {u.joinDate}</p>
                </div>
                <span style={{ fontSize:12, color:'#64748B' }}>🪙 {u.coins}</span>
                <span style={{ fontSize:12, color:'#64748B' }}>Lv.{u.level}</span>
                ...
                <div style={{ display:'flex', gap:6 }}>
                  <button onClick={()=>handleViewAs(u)} style={{ background:'#EFF6FF', color:'#1D4ED8', border:'none', borderRadius:8, padding:'5px 10px', fontSize:11, fontWeight:700, cursor:'pointer' }}>👁 View As</button>
