...
export default function ImpactDashboard() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#F8FAFC' }}>
      <div style={{ background:'linear-gradient(135deg,#1E3A5F,#3B2A6B)', padding:'60px 32px', textAlign:'center' }}>
        <p style={{ fontFamily:'Poppins,sans-serif', fontWeight:900, color:'#D4AF37', fontSize:32, margin:'0 0 8px' }}>🌍 Live Impact</p>
      ...
      <div style={{ maxWidth:900, margin:'0 auto', padding:'32px 24px' }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))', gap:16, marginBottom:32 }}>
          {STATS.map((s,i)=>(
            <div key={s.label} style={{ background:'#fff', borderRadius:20, padding:24, textAlign:'center', border:'1.5px solid #E2E8F0', boxShadow:'0 2px 12px rgba(0,0,0,0.04)' }}>
              <p style={{ fontSize:32, margin:'0 0 8px' }}>{s.emoji}</p>
              <p style={{ fontFamily:'Poppins,sans-serif', fontWeight:900, color:'#1E3A5F', fontSize:28, margin:'0 0 4px' }}>
        ...
        <div style={{ background:'#fff', borderRadius:20, border:'1.5px solid #E2E8F0', padding:24 }}>
          <p style={{ fontFamily:'Poppins,sans-serif', fontWeight:800, color:'#1E3A5F', fontSize:18, marginBottom:16 }}>📍 States Reached — {STATES.length}/28</p>
          <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
            {STATES.map(s=>(
              <span key={s} style={{ background:'linear-gradient(135deg,#1E3A5F,#0F2140)', color:'#D4AF37', borderRadius:20, padding:'4px 12px', fontSize:12, fontWeight:600 }}>✓ {s}</span>
            ))}
          </div>
        </div>
      </div>
