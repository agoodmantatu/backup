  ...
  return (
    <AppLayout title="Switch Role">
      <div className="mb-8" style={{ textAlign:'center' }}>
        <h1 style={{ fontFamily:'Poppins,sans-serif', fontWeight:900, color:'#1E3A5F', fontSize:26, marginBottom:4 }}>Switch Your Role</h1>
        <p style={{ color:'#64748B', fontSize:15 }}>Currently: <strong style={{ color:'#D4AF37' }}>{user.role}</strong> — pick a different context to switch how you use TryIT</p>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))', gap:16, maxWidth:800, margin:'0 auto' }}>
        {ROLES.map(r=>(
          <button key={r.id} onClick={()=>handleSelect(r.id, r.home)}
            style={{ background: user.role===r.id ? 'linear-gradient(135deg,#1E3A5F,#0F2140)' : '#fff',
              border: user.role===r.id ? 'none' : '1.5px solid #E2E8F0',
            ...
            <p style={{ fontFamily:'Poppins,sans-serif', fontWeight:800, color: user.role===r.id?'#D4AF37':'#1E3A5F', fontSize:16, margin:'0 0 6px' }}>
              {r.label} {user.role===r.id && '✓'}
            </p>
            <p style={{ color: user.role===r.id?'rgba(255,255,255,0.7)':'#64748B', fontSize:13, lineHeight:1.5, margin:0 }}>{r.desc}</p>
          </button>
        ))}
      </div>
    </AppLayout>
