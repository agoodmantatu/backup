...
export default function Topbar({ onMenuClick, title = 'Dashboard' }) {
  ...
  const menuItems = [
    ...
    { label: '🎨 Themes',       action: () => navigate('/settings/themes') },
    { label: '💳 Pro Member',   action: () => navigate('/pro')      },
    { label: '🚪 Sign Out',     action: () => { logout(); navigate('/landing') }, red: true },
  ]

  const primaryExamName = user?.exams?.[0]?.name || 'Pick Your Exam'
    ...
    <header className="fixed top-0 left-0 lg:left-[260px] right-0 h-[68px] z-20 flex items-center justify-between px-5"
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="hidden md:flex items-center gap-2 glass-gold px-3 py-1.5">
          ...
          <span className="text-xs font-semibold text-[#1E3A5F]">{primaryExamName}</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-1.5 bg-slate-100 px-3 py-2 rounded-xl">
          ...
          <span className="text-sm font-bold text-[#1E3A5F]">{coinBalance.toLocaleString()}</span>
        </div>
        ...
        <div className="relative">
          <button onClick={() => setDropOpen(p => !p)}
            ...
            <span className="hidden md:block text-sm font-semibold text-slate-700">{firstName}</span>
            <ChevronDown size={14} className="text-slate-500" />
          </button>
          {dropOpen && (
            <>
              <div className="fixed inset-0 z-40" onClick={() => setDropOpen(false)} />
              <div className="absolute right-0 top-full mt-2 w-48 clay rounded-2xl p-2 shadow-xl z-50 bg-white">
                {menuItems.map(item => (
